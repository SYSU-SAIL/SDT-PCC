...
PYBIND11_MODULE(lidar_ops, m) {
    m.doc() = "LiDAR C++ ops loaded via torch.utils.cpp_extension";

    // feature extractor
    m.def("extract_features_with_segment", &extract_features_with_segment);
    m.def("extract_features", &extract_features);
    m.def("segment_index_clean", &segment_index_clean);

    // segmentation
    m.def("intra_predict", &intra_predict);
    m.def("point_modeling", &point_modeling);

    // quantization
    m.def("uniform_quantize", &uniform_quantize);
    m.def("nonuniform_quantize", &nonuniform_quantize);

    // contour
    m.def("extract_contour", &extract_contour);
    m.def("recover_map", &recover_map);

    // dataset utils
    m.def("point_cloud_to_range_image_even",
          &point_cloud_to_range_image_even);
}
